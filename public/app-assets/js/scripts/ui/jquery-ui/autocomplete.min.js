...
$(document).ready(function() {
    ...
    function split_r(val) { return val.split(/,\s*/) }

    function extractLast_r(term) { return split_r(term).pop() }
    ...
    var names = ["Jörn Zaefferer", "Scott González", "John Resig"],
        accentMap = { "á": "a", "ö": "o" },
        normalize = function(term) { for (var ret = "", i = 0; i < term.length; i++) ret += accentMap[term.charAt(i)] || term.charAt(i); return ret };
    $(".ac-accent-folding").autocomplete({
        source: function(request, response) {
            var matcher = new RegExp($.ui.autocomplete.escapeRegex(request.term), "i");
            response($.grep(names, function(value) { return value = value.label || value.value || value, matcher.test(value) || matcher.test(normalize(value)) }))
        }
    ...
    $(".ac-category").catcomplete({ delay: 0, source: data }), $.widget("custom.combobox", {
        _create: function() { this.wrapper = $("<div>").addClass("custom-combobox input-group").insertAfter(this.element), $('<div class="input-group-prepend"><span class="input-group-text"><i class="ft-search"></i></span></div>').appendTo(this.wrapper), this.element.hide(), this._createAutocomplete(), this._createShowAllButton() },
        _createAutocomplete: function() {
            var selected = this.element.children(":selected"),
                value = selected.val() ? selected.text() : "";
            this.input = $("<input>").appendTo(this.wrapper).val(value).attr("title", "").addClass("custom-combobox-input form-control ui-widget ui-widget-content ui-state-default ui-corner-left").autocomplete({ delay: 0, minLength: 0, source: $.proxy(this, "_source") }).tooltip({ classes: { "ui-tooltip": "ui-state-highlight" } }), this._on(this.input, { autocompleteselect: function(event, ui) { ui.item.option.selected = !0, this._trigger("select", event, { item: ui.item.option }) }, autocompletechange: "_removeIfInvalid" })
        },
        _createShowAllButton: function() {
            var input = this.input,
                wasOpen = !1;
            $('<div class="input-group-prepend"><span class="input-group-text ac-toggle"><i class="ft-chevron-down"></i></span></div>').attr("tabIndex", -1).appendTo(this.wrapper).removeClass("ui-corner-all").addClass("custom-combobox-toggle ui-corner-right").on("mousedown", function() { wasOpen = input.autocomplete("widget").is(":visible") }).on("click", function() { input.trigger("focus"), wasOpen || input.autocomplete("search", "") })
        },
        ...
        _removeIfInvalid: function(event, ui) {
            if (!ui.item) {
                var value = this.input.val(),
                    ...
                    valid = !1;
                this.element.children("option").each(function() { if ($(this).text().toLowerCase() === valueLowerCase) return this.selected = valid = !0, !1 }), valid || (this.input.val("").attr("title", value + " didn't match any item").tooltip("open"), this.element.val(""), this._delay(function() { this.input.tooltip("close").attr("title", "") }, 2500), this.input.autocomplete("instance").term = "")
            }
        },
