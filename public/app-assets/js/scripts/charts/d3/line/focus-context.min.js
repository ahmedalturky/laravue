...
$(window).on("load", function() {
    function brushed() { x.domain(brush.empty() ? x2.domain() : brush.extent()), focus.select(".d3-area").attr("d", area), focus.select("d3-xaxis").call(xAxis) }

    function type(d) { return d.date = parseDate(d.date), d.price = +d.price, d }
    ...
    var parseDate = d3.time.format("%b %Y").parse,
        ...
        area = d3.svg.area().interpolate("monotone").x(function(d) { return x(d.date) }).y0(height).y1(function(d) { return y(d.price) }),
        area2 = d3.svg.area().interpolate("monotone").x(function(d) { return x2(d.date) }).y0(height2).y1(function(d) { return y2(d.price) }),
        container = ele.append("svg"),
        svg = container.attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
