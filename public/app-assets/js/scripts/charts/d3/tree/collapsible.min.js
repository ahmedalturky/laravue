...
$(window).on("load", function() {
    function update(source) {
        ...
        var link = svg.selectAll(".d3-tree-link").data(tree.links(nodes), function(d) { return d.target.id });
    ...
    function toggle(d) { d.children ? (d._children = d.children, d.children = null) : (d.children = d._children, d._children = null) }
    var root, ele = d3.select("#collapsible-tree"),
    ...
    d3.json("app-assets/data/d3/tree/collapsible-tree.json", function(error, json) {
        function toggleAll(d) { d.children && (d.children.forEach(toggleAll), toggle(d)) }
        root = json, root.x0 = height / 2, root.y0 = 0, root.children.forEach(toggleAll), toggle(root.children[1]), toggle(root.children[1].children[2]), toggle(root.children[9]), toggle(root.children[9].children[0]), update(root)
    })
});